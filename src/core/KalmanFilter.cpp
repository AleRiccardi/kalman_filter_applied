...
void KalmanFilter::update_H_gps() {
  ...
  R_.setZero();
  R_(0, 0) = std::pow(params_->noise_gps[0], 2);
  R_(1, 1) = std::pow(params_->noise_gps[1], 2);
  R_(2, 2) = std::pow(params_->noise_gps[2], 2);
}

void KalmanFilter::update_H_radar() {
  double x = params_->init_pose_radar(0, 0) - state_(0, 0);
  double y = params_->init_pose_radar(1, 0) - state_(1, 0);
  double z = params_->init_pose_radar(2, 0) - state_(2, 0);
  double x_2 = std::pow(x, 2);
  double y_2 = std::pow(y, 2);
  double z_2 = std::pow(z, 2);
  double ro = std::pow(std::pow(x, 2) + std::pow(y, 2) + std::pow(z, 2), 0.5);
